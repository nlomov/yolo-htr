...
class OBBPredictor(DetectionPredictor):
    ...
    def postprocess(self, preds, img, orig_imgs, true_preds=None):
        ...
        for pred, orig_img, img_path in zip(preds, orig_imgs, self.batch[0]):
            ...
            charset = sorted(self.model.model.charset)
            lines = ops.decode_probs(char_probs, charset)
            '''
            if self.args.latin_only:
                k = len(self.model.model.charset) // self.model.model.ng
                idx = ['LETTER' in unicodedata.name(c) and 'LATIN' not in unicodedata.name(c) for c in \
                       list(self.model.model.charset)[:k]] * self.model.model.ng + [False]
                char_probs[:,:,-1][char_probs.amax(axis=-1) != char_probs[:,:,-1]] = -float('inf')
                char_probs[:,:,idx] = -float('inf')
            lines0 = []
            for i in range(len(lines)):
                lines[i] = ''.join(chr(ord(c) % 10000) + \
                               ('' if ord(c) < 10000 else (chr(0)+chr(818)+chr(821)+chr(819))[ord(c) // 10000]) for c in lines[i])
                lines0.append(''.join(charset[j] if j < len(charset) else chr(0) for j in char_probs[i].argmax(axis=1)))
            results[-1].lines0 = lines0
            '''
