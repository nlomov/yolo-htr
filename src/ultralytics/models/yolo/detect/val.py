...
from ultralytics.utils.metrics import ConfusionMatrix, DetMetrics, box_iou, batch_probiou, iou_rotated
from ultralytics.utils.plotting import output_to_target, plot_images
from ultralytics.utils.ops import format_string_for_wer, scores_by_box, decode_probs
import itertools

from editdistance import distance as editdistance
from ultralytics.utils.metrics import editdistance_chars, editdistance_words, get_cer_and_wer
...
class DetectionValidator(BaseValidator):
    ...
    def get_desc(self):
        ...
        return ("%22s" + "%11s" * 8) % ("Class", "Images", "Instances", "Box(P", "R", "mAP50", "mAP50-95)", "CER", "WER")

    def postprocess(self, preds, true_bboxes=None):
        """Apply Non-maximum suppression to prediction outputs."""
        predn = ops.non_max_suppression(
            preds,
            self.args.conf,
            self.args.iou,
            labels=self.lb,
        ...
        if true_bboxes is None:
            pred_bboxes = predn.copy()
        else:
            pred_bboxes = []
            boxes, idx = true_bboxes
            for i in range(len(predn)):
                mask = idx == i
                pred_bboxes.append(torch.hstack([boxes[mask,:2] - boxes[mask,2:4]/2, boxes[mask,:2] + boxes[mask,2:4]/2, \
                                   torch.ones_like(boxes[mask,:1]),torch.zeros_like(boxes[mask,:1])]))
        ...
        return predn, pred_enc, char_probs
                ...
                if self.args.ctc > 0:
                    ...
                    if not self.args.save_preds:
                        ...
                        if self.args.task == 'obb':
                            cost_matrix = iou_rotated(bbox, predn[predn[:,4] >= conf_thr][:,(0,1,2,3,6)]).detach().cpu().numpy()
                        else:
                            cost_matrix = box_iou(bbox[:,:4], predn[predn[:,4] >= conf_thr,:4]).detach().cpu().numpy()
