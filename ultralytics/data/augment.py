...
from shapely import Polygon, convex_hull
...
class Mosaic(BaseMixTransform):
    ...
    def _cat_labels(self, mosaic_labels):
        ...
        for labels in mosaic_labels:
            ...
            lines.append(labels["lines"])
            bounds.append(labels["bounds"])
        ...
        prev_bboxes = final_labels["instances"].bboxes.copy()   # l,t,r,b
        final_labels["instances"].clip(imgsz, imgsz)
        good = final_labels["instances"].remove_zero_area_boxes()
        
        prev_bboxes = prev_bboxes[good]
        new_bboxes = final_labels["instances"].bboxes
        final_labels["cls"] = final_labels["cls"][good]
        final_labels["lines"] = [f for f,g in zip(final_labels["lines"],good) if g]
        final_labels["bounds"] = [f for f,g in zip(final_labels["bounds"],good) if g]
        
        width  = new_bboxes[:,2] - new_bboxes[:,0]
        height = new_bboxes[:,3] - new_bboxes[:,1]
        left  = (prev_bboxes[:,0] - new_bboxes[:,0]) / np.maximum(width, 1e-6)
        right = (prev_bboxes[:,2] - new_bboxes[:,0]) / np.maximum(width, 1e-6)
        up    = (prev_bboxes[:,1] - new_bboxes[:,1]) / np.maximum(height, 1e-6)
        down  = (prev_bboxes[:,3] - new_bboxes[:,1]) / np.maximum(height, 1e-6)
        
        for i in range(len(final_labels["bounds"])):
            final_labels["bounds"][i] = np.vstack([left[i] + (right[i]-left[i])*final_labels["bounds"][i][0], \
                                                   up[i]   + (down[i] -up[i]  )*final_labels["bounds"][i][1]])
...
class MixUp(BaseMixTransform):
    ...
    def _mix_transform(self, labels):
        ...
        labels["cls"] = np.vstack([labels["cls"], labels2["cls"]])
        labels["lines"] = labels["lines"] + labels2["lines"]
        labels["bounds"] = labels["bounds"] + labels2["bounds"]
        return labels
...
class RandomPerspective:
    ...
    def apply_bboxes(self, bboxes, M):
        ...
        frame = Polygon( ((0, 0), (0, self.size[1]), (self.size[0], self.size[1]), (self.size[0], 0)) )
        pgons = [convex_hull(Polygon( ((c[0],c[1]), (c[4],c[5]), (c[2],c[3]), (c[6],c[7])) )) for c in xy]
        pgons = [p.intersection(frame) for p in pgons]
        
        xmin = [np.array(p.exterior.coords.xy[0]).min() if p.area > 0 else -1 for p in pgons]
        ymin = [np.array(p.exterior.coords.xy[1]).min() if p.area > 0 else -1 for p in pgons]
        xmax = [np.array(p.exterior.coords.xy[0]).max() if p.area > 0 else -1 for p in pgons]
        ymax = [np.array(p.exterior.coords.xy[1]).max() if p.area > 0 else -1 for p in pgons]
        
        # Create new boxes
        return np.concatenate((xmin, ymin, xmax, ymax), dtype=bboxes.dtype).reshape(4, n).T
    ...
    def apply_segments(self, segments, M):
        ...
        frame = Polygon( ((0, 0), (0, self.size[1]), (self.size[0], self.size[1]), (self.size[0], 0)) )
        pgons = [convex_hull(Polygon(xy)) for xy in segments]
        npts = segments.shape[1] + 4
        
        pgons = [p.intersection(frame) for p in pgons]
        xy = []
        for p in pgons:
            if p.area == 0:
                xy.append(-np.ones((npts,2), dtype='float32'))
            else:
                if not isinstance(p, Polygon):
                    p0 = Polygon(-np.ones((3,2)))
                    for g in p.geoms:
                ...
                x,y = x[:npts],y[:npts]
                xy.append(np.array([[a,b] for a,b in zip(x,y)] + [[x[0],y[0]] for _ in range(npts-len(x))], dtype='float32'))
        segments = np.array(xy)
        
        bboxes = np.stack([segments[...,0].min(axis=1), segments[...,1].min(axis=1), segments[...,0].max(axis=1), segments[...,1].max(axis=1)]).transpose()
    ...
    def __call__(self, labels):
        ...
        for i in range(len(bounds)):
            x = instances.bboxes[i,0] + (instances.bboxes[i,2] - instances.bboxes[i,0]) * bounds[i][0]
            y = instances.bboxes[i,1] + (instances.bboxes[i,3] - instances.bboxes[i,1]) * bounds[i][1]
            C = np.vstack((x, y, np.ones_like(x))).T @ M.T
            x = C[:,0] / C[:,2]
            y = C[:,1] / C[:,2]
            bounds[i] = np.vstack([(x - bboxes[i,0]) / np.maximum(bboxes[i,2] - bboxes[i,0], 1e-6), \
                                   (y - bboxes[i,1]) / np.maximum(bboxes[i,3] - bboxes[i,1], 1e-6)])
        ...
        i = self.box_candidates(
            box1=instances.bboxes.T, box2=new_instances.bboxes.T, area_thr=0.01 if len(segments) else 1e-6
        )
        
        labels["instances"] = new_instances[i]
        labels["cls"] = cls[i]
        labels["lines"] = [l for l,g in zip(lines,i) if g]
        labels["bounds"] = [l for l,g in zip(bounds,i) if g]
        labels["img"] = img
...
class RandomFlip:
    ...
    def __call__(self, labels):
        ...
        if self.direction == "vertical" and random.random() < self.p:
            img = np.flipud(img)
            instances.flipud(h)
            labels["bounds"] = [np.vstack([b[0], 1 - b[1]]) for b in labels["bounds"]]
        if self.direction == "horizontal" and random.random() < self.p:
            img = np.fliplr(img)
            instances.fliplr(w)
            labels["lines"] = [l[::-1] for l in labels["lines"]]
            labels["bounds"] = [np.vstack([1 - b[0,::-1], b[1,::-1]]) for b in labels["bounds"]]
            # For keypoints
    ...
    def __call__(self, labels):
        ...
        if self.p and len(instances.segments):
            ...
            for j in random.sample(list(indexes), k=round(self.p * n)):
                ...
                lines = lines + [l for l,g in zip(lines,j) if j]
                bounds = bounds + [b for b,g in zip(bounds,j) if j]
    ...
    def __call__(self, labels):
        ...
        if len(cls):
            ...
            if self.transform and random.random() < self.p:
                ...
                if len(new["class_labels"]) > 0:  # skip update if no bbox in new image
...
class Format:
    ...
    def __call__(self, labels):
        ...
        if self.return_mask:
            labels["lines"] = [labels["lines"][i] for i in sorted_idx] if nl else []
            labels["bounds"] = [labels["bounds"][i] for i in sorted_idx] if nl else []
        ...
        if self.return_obb:
            if len(instances.segments):
                xy = torch.from_numpy(instances.segments)
                if instances.normalized:
                    xy *= np.array([w,h],dtype='float32')
                xy = xyxyxyxy2xywhr(xy)      
                mask = xy[:, 4] > torch.pi/4
                xy[mask,2:4] = xy[mask][:,(3,2)]
                xy[mask,4] -= torch.pi/2
                if instances.normalized:
                    xy /= torch.tensor([w,h,w,h,1]).to(xy.device)
    ...
    def _format_segments(self, instances, cls, w, h):
        ...
        return masks, instances, cls, sorted_idx
